...
struct bpf_map_def SEC("maps") pending_mutations = {
    .type = BPF_MAP_TYPE_HASH,
    .key_size = sizeof(unsigned int),
    .value_size = sizeof(struct pending_mutation),
    .max_entries = 128,
};
...
static __always_inline int remember_io(void *map, struct trace_sys_enter *ctx)
{
    unsigned int tid;
...
static __always_inline int submit_io(void *map, struct trace_sys_exit *ctx, unsigned int kind)
{
    ...
    event->arg0 = event->arg1 = event->arg2 = event->arg3 = 0;
    ...
    if (captured > 0) {
        int error = bpf_probe_read_user(event->data, captured, (void *)pending->user_buffer);
        if (error) {
...
static __always_inline int submit_mutation_immediate(unsigned int kind)
{
    unsigned long long pid_tgid;
    unsigned int tid;
    struct event *event;
...
int enter_read(struct trace_sys_enter *ctx) { return remember_io(&pending_reads, ctx); }
SEC("tracepoint/syscalls/sys_exit_read")
int exit_read(struct trace_sys_exit *ctx) { return submit_io(&pending_reads, ctx, KIND_READ); }
SEC("tracepoint/syscalls/sys_enter_write")
int enter_write(struct trace_sys_enter *ctx) { return remember_io(&pending_writes, ctx); }
SEC("tracepoint/syscalls/sys_exit_write")
int exit_write(struct trace_sys_exit *ctx) { return submit_io(&pending_writes, ctx, KIND_WRITE); }

SEC("kprobe/vfs_rename")
int kprobe_vfs_rename(struct pt_regs *ctx) { return submit_mutation_immediate(KIND_RENAME); }

SEC("kprobe/vfs_unlink")
int kprobe_vfs_unlink(struct pt_regs *ctx) { return submit_mutation_immediate(KIND_UNLINK); }

SEC("kprobe/chmod_common")
int kprobe_chmod_common(struct pt_regs *ctx) { return submit_mutation_immediate(KIND_CHMOD); }
