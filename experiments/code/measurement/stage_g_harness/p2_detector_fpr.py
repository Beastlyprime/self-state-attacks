...
_PROFILE_TOKEN_RE = re.compile(r"_(w[1-4])_", re.IGNORECASE)


def profile_from_run_id(run_id: str | None) -> str | None:
    """Return the case-declared workload profile (W1..W4) encoded in a run_id.

    Landers embed their profile as a ``_w4_`` token in the run_id. That token is
    the authoritative, case-declared profile; the on-disk directory can misfile a
    lander under the wrong profile (e.g. a W4 lander physically stored under
    ``attack/W3/``). Returns the uppercased token or ``None`` when absent.
    """
    if not run_id:
        return None
    match = _PROFILE_TOKEN_RE.search(run_id)
    return match.group(1).upper() if match else None


def reconcile_record_profile(record: dict[str, Any]) -> dict[str, Any]:
    """Return a copy of *record* with ``profile`` derived from run_id metadata.

    The raw ``profile`` in ``input_inventory.json`` is the on-disk directory,
    which can misfile a lander. The run_id ``_w4_`` token wins when present and
    disagreeing. ``dir_profile`` preserves the on-disk location for physical path
    construction; ``profile_source`` records provenance. Records without a token
    are returned unchanged apart from the ``dir_profile``/``profile_source``
    annotations, so correctly-filed corpora are unaffected.
    """
    out = dict(record)
    dir_profile = record.get("profile")
    out["dir_profile"] = dir_profile
    token = profile_from_run_id(record.get("run_id"))
    if token and token != dir_profile:
        out["profile"] = token
        out["profile_source"] = "run_id_token_override_dir"
    elif token:
        out["profile_source"] = "run_id_token"
    else:
        out["profile_source"] = "directory"
    return out
...
def score_stide(args: argparse.Namespace) -> None:
    ...
    for record in heldout_records(args.input_root):
        ...
        # another profile's model. A profile with no training baseline (or no
        # frozen-core definition) is data_insufficient, not scored cross-profile.
        if not train or profile not in stop.get("profile_freeze", {}):
            rows.append({
                "run_id": run_id, "profile": profile, "dir_profile": dir_profile,
                "branch_outcome": record["branch_outcome"], "status": "data_insufficient",
        ...
        out = args.output / "runs" / run_id
        out.mkdir(parents=True)
        test = args.input_root / "heldout" / dir_profile / run_id / "graph" / "syscalls.jsonl"
...
def score_unicorn(args: argparse.Namespace) -> None:
    ...
    for profile in ("W1", "W2", "W3", "W4"):
        ...
        if {path.name for path in test_dir.glob("*.txt")} != expected_heldout:
            raise RuntimeError(f"{profile}: missing heldout UNICORN sketches")
        model_cmd = [
            "docker", "run", "--rm", "--user", uid, "-e", "HOME=/tmp",
        ...
        grid = parse_unicorn_grid(model_run.stdout, expected_heldout)
        write_json(profile_out / "parsed_grid.json", grid)
        heldout_meta = {row["run_id"]: row for row in records if row["role"] == "heldout"}
        for filename, verdict in grid["3.0"].items():
            run_id = filename[:-4]
            row = heldout_meta[run_id]
            all_rows.append({
                "run_id": run_id, "profile": profile,
